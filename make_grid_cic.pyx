...
def makeGridWithCICPBC(float[:] x, float[:] y, float[:] z, float[:] masses, float L_BOX, int N):
    ...
    assert x.shape[0] == y.shape[0] and y.shape[0] == z.shape[0] and z.shape[0] == masses.shape[0] # Since we disable boundscheck.
...
def inverseCICTidalPBC(float[:] x, float[:] y, float[:] z, float[:,:,:,:,:] tidal, float L_BOX, int N, traceless = False, normalize = False):
    ...
    # Consistency
    assert x.shape[0] == y.shape[0] and y.shape[0] == z.shape[0] # Since we disable boundscheck.
